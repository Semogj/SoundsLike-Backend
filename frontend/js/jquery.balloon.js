...
(function($) {
        ...
        if (options.tipSize > 0) {
            ...
            for (var i = 0; i < Meta.pos.length; i++) {
                ...
                if (balloon.center[m.pos.c1] >= target[m.pos.c1] &&
                    ...
                    if (i % 2 == 0) {
                        if (balloon[m.pos.o] >= target[m.pos.o] && balloon[m.pos.f] >= target[m.pos.f])
                            break;
                    } else {
                        if (balloon[m.pos.o] <= target[m.pos.o] && balloon[m.pos.f] <= target[m.pos.f])
                            break;
    ...
    $.fn.showBalloon = function(options) {
        ...
        return this.each(function() {
            ...
            if (isNew)
                $balloon = $("<div>").append(contents);
            if (!options.url && (!$balloon || $balloon.html() == ""))
                return;
            if (!isNew && contents && contents != $balloon.html())
                $balloon.empty().append(contents);
            $target.removeAttr("title");
            if (options.url) {
                $balloon.load($.isFunction(options.url) ? options.url(this) : options.url, function(res, sts, xhr) {
                    if (options.ajaxComplete)
                        options.ajaxComplete(res, sts, xhr);
    ...
    $.balloon = {
        defaults: {
            ...
            hideDuration: 80, hideAnimation: function(d) {
                this.fadeOut(d);
            },
            css : {
                minWidth: "20px",
