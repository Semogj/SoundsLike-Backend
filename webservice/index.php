...
{
    showErrorResponse(HTML_500_INTERNAL_SERVER_ERROR, 'Internal Server Error', 'Shit just happened!',
...
if (version_compare(PHP_VERSION, MINIMUM_PHP_VERSION, '<='))
{
    ...
    showErrorResponse(HTML_500_INTERNAL_SERVER_ERROR, 'Internal Server Error', 'Shit just happened!',
                      "Server PHP 5.3.0 version or higher is required!");
}

try {
    $db = new \PDO("{$config['dbDriver']}:host={$config['dbHost']};dbname={$config['dbName']}", $config['dbUser'],
                   $config['dbPassword']);
...
try {
    ...
    showErrorResponse(HTML_500_INTERNAL_SERVER_ERROR, 'Database Error',
                      'Something went wrong with the connection to the database.', $error);
} catch (\Exception $ex) {
    $error = 'Fatal error: ' . $ex;
    $logger->LogFatal($error);
    showErrorResponse(HTML_500_INTERNAL_SERVER_ERROR, 'Server General Error', 'Something went wrong with the server.',
                      $error);
}
?>