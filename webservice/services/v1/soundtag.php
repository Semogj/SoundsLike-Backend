...
//        $this->videoModel = new \VIRUS\webservice\models\VideoModel();
    ...
    public function post(WebserviceRequest $request)
    {
        ...
        {
            if (empty(UserModel::getSingle($userId)))
            {
        ...
        {
            if (empty(UserModel::getSingle($segmentId)))
            {
        ...
        if(!empty($errorArr)){
            ...
            if(!$result){
                ...
                return new WebserviceOkResponse($request->getAcceptType(),HTML_200_OK,array('insertId' => $result));
            }
        }
